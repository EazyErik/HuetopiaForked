...
export const ColorPaletteDetails = (props) => {
  ...
  useEffect(() => {
    axios
      .get(
        `https://json-server-backend-xys5.onrender.com/${props.url}/${paletteId}`
      )
      .then((response) => {
        // console.log(response.data);
        setPalette(response.data);
        return axios.get(
          `https://json-server-backend-xys5.onrender.com/favourites`
        );
      })
      .then((response) => {
  ...
  const favouriteHandler = (event) => {
    ...
    if (!favourite) {
      ...
      axios
        .delete(
          `https://json-server-backend-xys5.onrender.com/favourites/${paletteId}`
        )
        .then(() => {
          ...
          <div className="flex justify-between">
            ...
            {favourite === true ? (
              <div className="flex gap-5 items-center" onClick={editName}>
                {editable ? (
                  <button className="btn btn-xs" onClick={updateHandler}>
                    Update
                  </button>
                ) : (
